...
- 여기서 userToken이 없거나 로그인이 안된 상태에서 자신이 작성한 포스트를 수정 또는 삭제하려고 시도한다면 권한 에러를 발생시킵니다.
...
- 로그인하지 않은 불 특정 사용자가 다른 유저 포스트에 좋아요를 누르거나 수정 또는 삭제하려고 시도할 때, fetch 메서드를 활용해 localstorge에 유저 토큰의 유무를 확인 및 404
권한 에러를 발생시키도록 기능을 수정하여, 로그인이 안된 상태에서도 포스트를 작성하거나 수정 또는 삭제 할 경우에는, 로그인 후 이용해달라는 알림 문구를 띄워서 문제를 보완했습니다.
...
- 팀 프로젝트에 참여하면서 배웠던 스택들을 스스로 잘 적용했는지 점검하기 위한 목적으로 진행하였습니다.
